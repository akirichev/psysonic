export const login = {
  subtitle: 'Вашият Navidrome настолен плеър',
  serverName: 'Име на сървър (по избор)',
  serverNamePlaceholder: 'Моят Navidrome',
  serverUrl: 'URL на сървъра',
  serverUrlPlaceholder: '192.168.1.100:4533 или https://music.example.com',
  username: 'Потребителско име',
  usernamePlaceholder: 'admin',
  password: 'Парола',
  showPassword: 'Покажете парола',
  hidePassword: 'Скрийте парола',
  connect: 'Свържете се',
  connecting: 'Свързване…',
  connected: 'Свързан!',
  error: 'Неуспешно свързване – моля, проверете данните си.',
  urlRequired: 'Моля, въведете URL на сървъра.',
  savedServers: 'Запазени сървъри',
  addNew: 'Или добавете нов сървър',
  orMagicString: 'Или магически код',
  magicStringPlaceholder: 'Поставете код за споделяне (psysonic1-…)',
  magicStringInvalid: 'Невалиден или нечетлив магически код.',
};
